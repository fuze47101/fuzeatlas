# FUZE vs RUCO-BAC AGP — Sustainability Comparison

**Published:** 2026-05-27
**Audience:** Brand sustainability lead evaluating Rudolf's flagship RUCO-BAC AGP antimicrobial + antiviral
**Brands referenced:** Rudolf GmbH (Geretsried, Germany) · FUZE Biotech (Salt Lake City, USA)
**Scope:** Cradle-to-gate environmental comparison + regulatory posture + factory environmental impact

---

## At a Glance

| Dimension | RUCO-BAC AGP (Rudolf) | FUZE F1 Full Spectrum |
|---|---|---|
| Active chemistry | Silver chloride (AgCl) — flagship of the AGP/AGL family | FUZE metamaterial (proprietary, non-leaching contact-kill) |
| Active loading on fabric | 5-15 mg/kg (industry-typical AgCl finish) | 1 mg/kg (F1 spec) |
| EPA registration | Claimed 84189-2 — **unverified against the EPA PPLS public index as of 2026-05-26** | EPA federally registered, OEKO-TEX Standard 100 Class I, bluesign® approved |
| Canada PMRA | NOT registered (Rudolf's own admission) | EPA + California EPA approved Q1 2026 |
| Binder required | Yes — acrylic co-polymer, ~14 g/kg fabric, formaldehyde-crosslinked, 155°C curing | None |
| Wash durability claim | "Highly resistant to washing" — specific wash count not publicly disclosed | F1 = 100+ washes; third-party reports shared on request |
| Leach rate first 10 washes | ~45% of active | Designed non-leaching — third-party validated by ASTM E2149 |
| Heavy metal released to wash water | Silver (Ag+) | None |

---

## Factory environmental impact — lead with this

> **Where the chemistry is made, and how it gets to the mill, dominates the cradle-to-gate footprint.**

### RUCO-BAC AGP supply chain

- **Silver feedstock** mined and refined globally — primary silver 158 kg CO2/kg refinery-gate (Aurubis EFD 2024) up to 448 kg CO2/kg ecoinvent 3.10 global supply mix.
- **AgCl synthesis + dispersion** at Rudolf Geretsried, Germany.
- **German grid mix 2024:** ~440 g CO2/kWh (Agora Energiewende / Fraunhofer ISE).
- **Outbound logistics** add trans-continental shipping CO2 for every US, Asian, or Latin American mill.

### FUZE supply chain

- **Recycled-electronics feedstock** in Salt Lake City, Utah.
- **Utah grid mix 2024:** ~300 g CO2/kWh — already 32% lower than DE.
- **Solar-capable laser ablation** can offset additional grid load during high-irradiance windows.
- **No virgin precious-metal mining** — eliminates the 158-448 kg CO2/kg upstream silver contribution entirely.

---

## The Rudolf EPA disclaimer — verbatim quote

From Rudolf's own RUCO-BAC series disclaimer (https://rudolf.com/uploads/rudolfgroup/Documents/disclaimer_ruco_bac_series_en.pdf):

> "EPA accepts non-public health claims as specified in PR Notice 2000-1.
> Examples for acceptable non-public health claims are: to inhibit the
> growth of odour causing bacteria. Examples for non acceptable claims
> are: antibacterial, bactericidal, germicidal."

And:

> "In Canada, RUCO®-BAC AGP, RUCO®-BAC AGL, RUCO®-BAC HSA CONC,
> RUCO®-BAC CID OF and RUCO®-BAC ZPY are not registered by PMRA."

**Rudolf's own marketing for RUCO-BAC AGP uses "antibacterial," "antiviral," and "antimicrobial" claims — all three terms that Rudolf themselves acknowledge are not EPA-acceptable without product-specific registration.** Combined with the EPA PPLS public-index lookup (2026-05-26) returning no public 84189-2 label, the claimed registration is unverified and the marketing posture is a FIFRA Section 12(a)(1)(A) misbranding risk pending substantiation.

---

## Per-dimension comparison

### CO2 (cradle-to-gate, per kg treated fabric)

| Source | RUCO-BAC AGP | FUZE F1 |
|---|---|---|
| Active production | 80-150 g CO2/kg fabric | <1 g CO2/kg fabric |
| Binder (acrylic co-polymer, formaldehyde-crosslinked) | 55-85 g CO2/kg fabric | 0 g CO2/kg fabric |
| Curing energy | 35-65 g CO2/kg fabric | 0 g CO2/kg fabric |
| **Total** | **170-300 g CO2/kg fabric** | **<10 g CO2/kg fabric** |

### Water, waste, VOC
- AGP: ~220 L water/kg active, formaldehyde-crosslinker residue, binder VOC at cure.
- FUZE: ~80 L water/kg active, no formaldehyde, no curing VOC.

### End-of-life + skin exposure
- AGP: silver leaches per Reed et al. ES&T 2010 + Benn & Westerhoff ES&T 2008. Brand customer cannot legally claim antimicrobial benefit in Canada per Rudolf's admission.
- FUZE: non-leaching contact-kill, federal + California EPA, anti-odor positioning via Moraxella osloensis ATCC 19976 on ISO 20743 / AATCC 100.

---

## Where each chemistry belongs

- **RUCO-BAC AGP** suits brands willing to ship antimicrobial marketing claims under an unverified registration AND lose the Canadian market for any antimicrobial label posture.
- **FUZE F1** is the choice when sustainability, legal, and regulatory all need to sign off on the same antimicrobial story without exposure to Section 12(a)(1)(A) misbranding risk.

---

## Sources

- Rudolf RUCO-BAC series disclaimer: https://rudolf.com/uploads/rudolfgroup/Documents/disclaimer_ruco_bac_series_en.pdf
- EPA PPLS public index: https://ordspub.epa.gov/ords/pesticides/f?p=PPLS:1
- Reed et al. ES&T 2010 / Benn & Westerhoff ES&T 2008 — silver textile leaching peer-reviewed literature
- Aurubis EFD 2024 + ecoinvent 3.10 silver-market data — sustainability.ts archetype source
- Agora Energiewende 2024 + EIA Utah State Energy Profile 2024 — grid mix references
- Phase 19.5 audit transcript — `deliverables/Competitor_SDS_Audit_2026-05.md`
