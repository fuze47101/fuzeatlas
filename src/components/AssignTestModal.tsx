...
export default function AssignTestModal({
  ...
  const handleSave = async () => {
    if (!brandId && !factoryId && !fabricId) {
      setError("Select at least one of brand, factory, or fabric");
      return;
    }
    setSaving(true);
    setError(null);
    ...
    try {
      const res = await fetch(`/api/tests/${testId}/assign`, {
        ...
        body: JSON.stringify({ brandId, factoryId, fabricId }),
      });
      ...
      <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-lg font-semibold text-slate-900">
                Assign Test to Brand / Factory / Fabric
              </h2>
              <p className="text-sm text-slate-500 mt-0.5">
                {testType} test{currentBrand ? ` — currently: ${currentBrand}` : ""}
        ...
        <div className="px-6 py-4 border-t border-slate-200 flex items-center justify-end gap-3">
          ...
          <button
            type="button"
            onClick={handleSave}
            disabled={saving || (!brandId && !factoryId && !fabricId)}
