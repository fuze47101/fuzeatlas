...
const STAGE_COLORS: Record<string,string> = {
  LEAD:"#6366f1", PRESENTATION:"#8b5cf6", BRAND_TESTING:"#0ea5e9",
  FACTORY_ONBOARDING:"#f59e0b", FACTORY_TESTING:"#f97316",
  PRODUCTION:"#22c55e", BRAND_EXPANSION:"#10b981", ARCHIVE:"#94a3b8",
  CUSTOMER_WON:"#059669",
};
...
export default function BrandDetailPage() {
  ...
  const [showLinkFactory, setShowLinkFactory] = useState(false);
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch(`/api/brands/${id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) });
      const j = await res.json();
      if (j.ok) { setBrand({ ...brand, ...j.brand }); setEditing(false); setSuccess("Brand updated successfully"); setTimeout(() => setSuccess(""), 3000); }
      else setError(j.error || "Failed to update brand");
    } catch (e: any) { setError(e.message); } finally { setSaving(false); }
  };

  const handleStageChange = async (newStage: string) => {
    try {
      const res = await fetch(`/api/brands/${id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ pipelineStage: newStage }) });
      const j = await res.json();
      if (j.ok) {
        setBrand({ ...brand, pipelineStage: newStage });
        setForm({ ...form, pipelineStage: newStage });
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const j = await res.json();
      if (j.ok) { router.push("/brands"); }
      else { setError(j.error || "Failed to delete brand"); setShowDeleteConfirm(false); }
  ...
  const loadTests = async () => {
    ...
    try {
      const res = await fetch(`/api/tests/by-entity?brandId=${id}`);
      const j = await res.json();
  ...
  const loadFactories = async () => {
    ...
    try {
      ...
      const j = await res.json();
      if (j.ok) setAllFactories(j.factories);
    } catch {}
  };
  ...
  const handleAddProduct = async () => {
    ...
    try {
      const res = await fetch(`/api/brands/${id}/products`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(productForm) });
      const j = await res.json();
      if (j.ok) { setProducts(prev => [j.product, ...prev]); setProductForm({ name: "", productType: "", sku: "", description: "" }); setShowAddProduct(false); }
  ...
  const handleUpdateProduct = async (productId: string) => {
    setProductSaving(true);
    try {
      const res = await fetch(`/api/products/${productId}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(editProductForm) });
      const j = await res.json();
      if (j.ok) { setProducts(prev => prev.map(p => p.id === productId ? { ...p, ...j.product } : p)); setEditingProduct(null); }
      else setError(j.error);
    } catch (e: any) { setError(e.message); } finally { setProductSaving(false); }
  };
  ...
  const handleResearch = async (autoSaveContacts = false) => {
    ...
    try {
      const res = await fetch(`/api/brands/${id}/research`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ autoSaveContacts }) });
      const j = await res.json();
      if (j.ok) {
        setResearch(j.research);
        setResearchDate(new Date().toISOString());
        if (autoSaveContacts) { setSuccess("Research complete — contacts saved!"); setTimeout(() => setSuccess(""), 4000); }
      } else { setResearchError(j.error || "Research failed"); }
    } catch (e: any) { setResearchError(e.message); } finally { setResearching(false); }
  };

  const handleLinkFactory = async () => {
    if (!linkFactoryId) return;
    try {
      const res = await fetch("/api/brand-factory", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ brandId: id, factoryId: linkFactoryId }) });
      const j = await res.json();
      if (j.ok) {
        setBrand({ ...brand, factories: [...brand.factories, j.link], _count: { ...brand._count, factories: brand._count.factories + 1 } });
        setShowLinkFactory(false); setLinkFactoryId("");
      } else setError(j.error);
    } catch (e: any) { setError(e.message); }
  };

  const handleUnlinkFactory = async (linkId: string) => {
    if (!confirm("Remove this factory link?")) return;
    try {
      const res = await fetch(`/api/brand-factory?id=${linkId}`, { method: "DELETE" });
      const j = await res.json();
      if (j.ok) {
        setBrand({ ...brand, factories: brand.factories.filter((f: any) => f.id !== linkId), _count: { ...brand._count, factories: brand._count.factories - 1 } });
      }
    } catch (e: any) { setError(e.message); }
  };
  ...
  const currentStageIdx = STAGES.indexOf(brand.pipelineStage);
  ...
  return (
    <div className="max-w-[1200px] mx-auto">
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <div className="flex items-center gap-3 mt-1">
            <span className="px-2 py-0.5 rounded-full text-xs font-bold text-white" style={{ background: STAGE_COLORS[brand.pipelineStage] || "#6366f1" }}>
      ...
      <div className="bg-white rounded-xl p-4 shadow-sm border mb-6">
        <div className="flex items-center gap-1">
          {STAGES.map((stage, idx) => {
            const isActive = stage === brand.pipelineStage;
            ...
            return (
              <button
                ...
                className={`flex-1 relative group transition-all ${idx === 0 ? "rounded-l-lg" : ""} ${idx === STAGES.length - 1 ? "rounded-r-lg" : ""}`}
                title={`Move to ${STAGE_LABELS[stage]}`}
              >
                <div
                  className={`h-8 flex items-center justify-center text-[10px] font-bold transition-all ${
                    isActive ? "text-white shadow-md scale-105 z-10" : isPast ? "text-white/90" : "text-slate-400 bg-slate-100"
                  }`}
                  style={{
                ...
                >
                  ...
                  <span className="lg:hidden">{STAGE_LABELS[stage].slice(0, 3)}</span>
                </div>
                <div className={`absolute -bottom-1 left-1/2 -translate-x-1/2 w-2 h-2 rounded-full transition-all ${isActive ? "bg-white border-2 scale-125" : "scale-0"}`} style={{ borderColor: isActive ? color : undefined }} />
              </button>
            );
          })}
        </div>
        ...
        {(["details","contacts","products","fabrics","factories","submissions","tests","sows","notes","research"] as const).map(tabKey => {
          ...
          return (
            <button key={tabKey} onClick={() => setTab(tabKey)}
              className={`px-3 py-2 text-sm font-semibold border-b-2 transition-colors whitespace-nowrap ${tab === tabKey ? "border-blue-600 text-blue-600" : "border-transparent text-slate-500 hover:text-slate-700"}`}>
              {tabLabels[tabKey]}
            </button>
          );
        })}
                ...
                {products.map((p: any) => (
                  editingProduct === p.id ? (
                    <tr key={p.id} className="border-b border-slate-100 bg-blue-50">
                      <td className="py-2 pr-2"><input type="text" value={editProductForm.name || ""} onChange={e => setEditProductForm({ ...editProductForm, name: e.target.value })} className="w-full px-2 py-1 border border-slate-300 rounded text-sm" /></td>
                      <td className="py-2 pr-2"><input type="text" value={editProductForm.productType || ""} onChange={e => setEditProductForm({ ...editProductForm, productType: e.target.value })} className="w-full px-2 py-1 border border-slate-300 rounded text-sm" /></td>
                      <td className="py-2 pr-2"><input type="text" value={editProductForm.sku || ""} onChange={e => setEditProductForm({ ...editProductForm, sku: e.target.value })} className="w-full px-2 py-1 border border-slate-300 rounded text-sm" /></td>
                      <td className="py-2 pr-2"><input type="text" value={editProductForm.description || ""} onChange={e => setEditProductForm({ ...editProductForm, description: e.target.value })} className="w-full px-2 py-1 border border-slate-300 rounded text-sm" /></td>
                    ...
                    <tr key={p.id} className="border-b border-slate-100 hover:bg-slate-50">
                      ...
                      <td className="py-2">{p.sows?.length || 0}</td>
                      <td className="py-2 text-right">
                        <button onClick={() => { setEditingProduct(p.id); setEditProductForm({ name: p.name, productType: p.productType || "", sku: p.sku || "", description: p.description || "" }); }} className="text-xs text-blue-600 hover:underline mr-2">{t.common.edit}</button>
                        <button onClick={() => { if (confirm(t.products.deleteConfirm)) handleDeleteProduct(p.id); }} className="text-xs text-red-500 hover:underline">{t.common.delete}</button>
                      </td>
                    </tr>
                  )
                ))}
      ...
      {tab === "fabrics" && (
        <div className="bg-white rounded-xl p-6 shadow-sm border">
          <div className="flex justify-between items-center mb-4">
            ...
            <button onClick={() => router.push(`/fabrics/new?brandId=${id}`)} className="px-3 py-1.5 bg-blue-600 text-white rounded-lg text-xs font-semibold hover:bg-blue-700">+ New Fabric</button>
          </div>
      ...
      {tab === "factories" && (
        <div className="bg-white rounded-xl p-6 shadow-sm border">
          <div className="flex justify-between items-center mb-4">
            <h3 className="font-bold text-slate-900">{t.brands.factories || "Linked Factories"}</h3>
            <button onClick={() => { setShowLinkFactory(!showLinkFactory); loadFactories(); }}
          ...
          {showLinkFactory && (
            <div className="mb-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
              <label className="block text-xs font-semibold text-slate-600 mb-1">Select Factory</label>
              <div className="flex gap-2">
                <select value={linkFactoryId} onChange={e => setLinkFactoryId(e.target.value)}
                ...
                <button onClick={handleLinkFactory} disabled={!linkFactoryId}
                  className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-semibold hover:bg-blue-700 disabled:opacity-50">Link</button>
                <button onClick={() => setShowLinkFactory(false)} className="px-4 py-2 text-sm text-slate-600 border border-slate-300 rounded-lg">{t.common.cancel}</button>
              </div>
            </div>
          )}
          {brand.factories.length === 0 ? (
            <p className="text-slate-400 text-sm text-center py-8">No factories linked to this brand yet.</p>
          ) : (
            <div className="space-y-2">
              {brand.factories.map((bf: any) => (
                <div key={bf.id} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg group hover:bg-blue-50">
                  <div className="flex items-center gap-3 cursor-pointer flex-1" onClick={() => router.push(`/factories/${bf.factory.id}`)}>
                    <span className="text-lg">🏭</span>
                    <div>
                      <div className="font-semibold text-sm text-slate-900">{bf.factory.name}</div>
                      <div className="text-xs text-slate-500">{bf.factory.country || "—"} {bf.isPrimary && <span className="ml-1 px-1.5 py-0.5 bg-green-100 text-green-700 text-[10px] font-bold rounded">PRIMARY</span>}</div>
                    </div>
                  </div>
                  <button onClick={() => handleUnlinkFactory(bf.id)} className="text-xs text-red-500 hover:underline opacity-0 group-hover:opacity-100 transition-opacity">Unlink</button>
                </div>
              ))}
            </div>
          )}
      ...
      {tab === "tests" && (
        <div className="bg-white rounded-xl p-6 shadow-sm border">
          <div className="flex justify-between items-center mb-4">
            ...
            <button onClick={loadTests} className="text-xs text-blue-600 hover:underline">Refresh</button>
          </div>
          {testsLoading ? (
            <p className="text-slate-400 text-sm text-center py-8">{t.common.loading}</p>
          ) : testRuns.length === 0 ? (
            <p className="text-slate-400 text-sm text-center py-8">No test results linked to this brand yet.</p>
          ) : (
            <table className="w-full text-sm">
              <thead><tr className="text-left text-xs text-slate-500 border-b">
              ...
              <tbody>
                {testRuns.map((tr: any) => {
                  const pass = tr.icpResult ? true : (tr.abResult?.methodPass ?? tr.abResult?.pass ?? tr.fungalResult?.pass ?? tr.odorResult?.pass ?? null);
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border">
            ...
            {research && !researching && (
              <div className="space-y-6">
                    ...
                    <div className="space-y-3">
                      {research.contacts.map((c: any, i: number) => (
                        <div key={i} className="flex items-start gap-3 p-3 bg-white rounded-lg border border-green-200">
                          <div className="w-10 h-10 bg-green-100 rounded-full flex items-center justify-center text-green-700 font-bold text-sm shrink-0">{(c.name || "?")[0]}</div>
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex gap-3 mt-1 text-xs">
                              {c.email && <span className="flex items-center gap-1"><a href={`mailto:${c.email}`} className="text-blue-600 hover:underline">{c.email}</a>{c.emailConfidence && <span className={`px-1 py-0 rounded text-[9px] font-bold ${c.emailConfidence === "verified" ? "bg-green-100 text-green-700" : c.emailConfidence === "likely" ? "bg-amber-100 text-amber-700" : "bg-slate-100 text-slate-500"}`}>{t.research[c.emailConfidence as 'verified' | 'likely' | 'estimated']}</span>}</span>}
                ...
                {research.opportunity && (
                  <div className="p-4 bg-purple-50 rounded-lg">
                    <h4 className="text-sm font-bold text-slate-800 mb-3 flex items-center gap-2">
                      ...
                      {research.opportunity.fitScore && <span className={`px-2 py-0.5 rounded-full text-xs font-bold ml-auto ${research.opportunity.fitScore >= 7 ? "bg-green-100 text-green-700" : research.opportunity.fitScore >= 4 ? "bg-amber-100 text-amber-700" : "bg-red-100 text-red-700"}`}>Fit: {research.opportunity.fitScore}/10</span>}
                      {research.opportunity.estimatedScale && <span className="px-2 py-0.5 bg-blue-100 text-blue-700 rounded-full text-xs font-bold">{research.opportunity.estimatedScale}</span>}
                    </h4>
                    {research.opportunity.fitReason && <p className="text-sm text-slate-700 mb-3">{research.opportunity.fitReason}</p>}
                    {research.opportunity.bestProductLines?.length > 0 && <div className="mb-2"><span className="text-xs font-bold text-slate-600">{t.research.bestProductLines}: </span><span className="text-xs text-slate-700">{research.opportunity.bestProductLines.join(", ")}</span></div>}
                    {research.opportunity.currentAntimicrobial && <div className="mb-2 p-2 bg-red-50 border border-red-200 rounded text-xs text-red-700"><strong>{t.research.currentAntimicrobial}:</strong> {research.opportunity.currentAntimicrobial}</div>}
                    {research.opportunity.suggestedApproach && <div className="mb-3 p-3 bg-white border border-purple-200 rounded-lg"><span className="text-xs font-bold text-purple-700 block mb-1">{t.research.suggestedApproach}</span><p className="text-sm text-slate-700">{research.opportunity.suggestedApproach}</p></div>}
                    {research.opportunity.openingMessage && <div className="p-3 bg-white border border-purple-200 rounded-lg"><span className="text-xs font-bold text-purple-700 block mb-1">{t.research.draftOpeningMessage}</span><p className="text-sm text-slate-700 italic">&ldquo;{research.opportunity.openingMessage}&rdquo;</p></div>}
                        ...
                        <div className="space-y-2">
                          {research.opportunity.objections.map((obj: any, i: number) => (
                            <div key={i} className="text-xs">
                              ...
                              <br /><span className="text-green-600 font-semibold">{t.research.counter}: </span><span className="text-slate-700">{obj.counter}</span>
                            </div>
                          ))}
                        </div>
                      </div>
                    ...
                    <div className="space-y-2">
                      {research.news.map((n: any, i: number) => (
                        <div key={i} className="text-sm">
                          <div className="flex items-center gap-2"><strong className="text-slate-900">{n.headline}</strong>{n.date && <span className="text-xs text-slate-400">{n.date}</span>}</div>
                ...
                {research.researchNotes && <div className="p-3 bg-slate-100 rounded-lg text-xs text-slate-600"><strong>{t.research.researchNotes}:</strong> {research.researchNotes}</div>}
              </div>
            )}
            {!research && !researching && (
              <div className="text-center py-12">
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-sm p-6">
            ...
            <p className="text-sm text-slate-600 mb-1">{t.brands.deleteConfirm} <strong>{brand.name}</strong>?</p>
            <p className="text-xs text-slate-500 mb-4">{t.brands.deleteWarning}</p>
            <div className="mb-4">
              ...
              <input type="password" value={adminCode} onChange={(e) => setAdminCode(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter" && adminCode) handleDelete(); }}
                className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-red-500" placeholder={t.brands.enterAdminCode} autoFocus />
            </div>
            <div className="flex justify-end gap-3">
              ...
              <button onClick={handleDelete} disabled={deleting || !adminCode} className="px-4 py-2 text-sm font-semibold text-white bg-red-600 rounded-lg hover:bg-red-700 disabled:opacity-50">{deleting ? t.common.deleting : t.brands.yesDelete}</button>
            </div>
          </div>
        </div>
      )}
...
function Field({ label, field, form, setForm, editing, type = "text", options, required, t }: any) {
  if (!editing) {
    let display = form[field] || "—";
    if (type === "select" && options) {
      const opt = options.find((o: any) => o.value === form[field]);
      display = opt?.label || form[field] || "—";
    }
    if (type === "date" && form[field]) {
      display = new Date(form[field]).toLocaleDateString();
    }
    return (
      <div>
...
function ContactsTab({ brandId, contacts: initial, onUpdate, t }: { brandId: string; contacts: any[]; onUpdate: (c: any[]) => void; t: any }) {
  ...
  const handleAdd = async () => {
    ...
    try {
      const res = await fetch("/api/contacts", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...form, brandId }) });
      const j = await res.json();
  ...
  return (
    <div className="bg-white rounded-xl p-6 shadow-sm border">
      <div className="flex justify-between items-center mb-4">
        ...
        <button onClick={() => { setShowAdd(!showAdd); setForm(empty); setEditingId(null); }} className="px-3 py-1.5 bg-blue-600 text-white rounded-lg text-xs font-semibold hover:bg-blue-700">+ {t.contacts.addContact}</button>
      </div>
      ...
      {showAdd && (
        <div className="mb-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
          ...
          <div className="flex gap-2">
            <button onClick={handleAdd} disabled={saving} className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-semibold hover:bg-blue-700 disabled:opacity-50">{saving ? t.common.saving : t.contacts.addContact}</button>
      ...
      {contacts.length === 0 && !showAdd ? (
        <p className="text-slate-400 text-sm text-center py-8">{t.brands.noContacts || "No contacts yet"}</p>
      ) : (
        <div className="space-y-2">
          {contacts.map((ct: any) => (
            editingId === ct.id ? (
              ...
              <div key={ct.id} className="flex items-center gap-4 p-3 bg-slate-50 rounded-lg group">
                <div className="w-10 h-10 bg-blue-100 rounded-full flex items-center justify-center text-blue-600 font-bold text-sm flex-shrink-0">{(ct.firstName || ct.name || "?")[0]}</div>
                <div className="flex-1 min-w-0">
...
function NotesTab({ brandId, notes: initial, onUpdate, t }: { brandId: string; notes: any[]; onUpdate: (n: any[]) => void; t: any }) {
  ...
  const sync = (updated: any[]) => { setNotes(updated); onUpdate(updated); };

  const handleAdd = async () => {
    if (!form.content.trim()) return;
    setSaving(true); setError("");
    try {
      const res = await fetch("/api/notes", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...form, brandId }) });
      const j = await res.json();
      if (j.ok) { sync([j.note, ...notes]); setForm(empty); setShowAdd(false); }
      else setError(j.error);
    } catch (e: any) { setError(e.message); } finally { setSaving(false); }
  };
  ...
  const handleUpdate = async (id: string) => {
    setSaving(true); setError("");
    try {
      const res = await fetch(`/api/notes/${id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) });
      const j = await res.json();
      if (j.ok) { sync(notes.map(n => n.id === id ? j.note : n)); setEditingId(null); }
      else setError(j.error);
    } catch (e: any) { setError(e.message); } finally { setSaving(false); }
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      const j = await res.json();
      if (j.ok) sync(notes.filter(n => n.id !== id));
    } catch (e: any) { setError(e.message); }
  };
    ...
    <div className="bg-white rounded-xl p-6 shadow-sm border">
      <div className="flex justify-between items-center mb-4">
        ...
        <button onClick={() => { setShowAdd(!showAdd); setForm(empty); setEditingId(null); }}
          className="px-3 py-1.5 bg-blue-600 text-white rounded-lg text-xs font-semibold hover:bg-blue-700">+ Add Note</button>
      </div>
      {error && <div className="mb-3 p-2 bg-red-50 border border-red-200 rounded-lg text-red-700 text-xs">{error}</div>}
      {showAdd && (
        <div className="mb-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
          ...
          <textarea value={form.content} onChange={e => setForm({ ...form, content: e.target.value })} rows={3}
            className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm mb-3 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Write your note..." autoFocus />
          <div className="flex gap-2">
            <button onClick={handleAdd} disabled={saving || !form.content.trim()}
              className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-semibold hover:bg-blue-700 disabled:opacity-50">{saving ? t.common.saving : "Save Note"}</button>
            <button onClick={() => setShowAdd(false)} className="px-4 py-2 text-sm text-slate-600 border border-slate-300 rounded-lg hover:bg-slate-50">{t.common.cancel}</button>
          </div>
        </div>
      )}
      {notes.length === 0 && !showAdd ? (
        ...
        <div className="space-y-3">
          {notes.map((n: any) => (
            editingId === n.id ? (
              <div key={n.id} className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                <div className="grid grid-cols-2 gap-3 mb-3">
                  <select value={form.noteType} onChange={e => setForm({ ...form, noteType: e.target.value })}
                  ...
                  <input type="text" value={form.contactName} onChange={e => setForm({ ...form, contactName: e.target.value })}
                    className="px-3 py-2 border border-slate-300 rounded-lg text-sm" placeholder="Contact name" />
                </div>
                <textarea value={form.content} onChange={e => setForm({ ...form, content: e.target.value })} rows={3}
                  className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm mb-3" />
                <div className="flex gap-2">
                  <button onClick={() => handleUpdate(n.id)} className="text-xs text-green-600 hover:underline font-semibold">{saving ? "..." : t.common.save}</button>
                  <button onClick={() => setEditingId(null)} className="text-xs text-slate-500 hover:underline">{t.common.cancel}</button>
                </div>
              </div>
            ) : (
              <div key={n.id} className="p-3 bg-slate-50 rounded-lg group">
                <div className="flex justify-between items-center mb-1">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => { setEditingId(n.id); setForm({ content: n.content || "", noteType: n.noteType || "NOTE", contactName: n.contactName || "" }); }} className="text-xs text-blue-600 hover:underline">{t.common.edit}</button>
                      <button onClick={() => handleDelete(n.id)} className="text-xs text-red-500 hover:underline">{t.common.delete}</button>
                    </div>
                  </div>
                </div>
