  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(
        `/api/brands/${deleteTarget.id}?code=${encodeURIComponent(adminCode)}`,
        { method: "DELETE" },
      );
  ...
  return (
    <div className="max-w-[1600px] mx-auto">
      ...
      <div className="flex gap-1 mb-6 rounded-xl overflow-hidden h-10">
        {STAGES.map((s) => {
          ...
          return (
            <div
              key={s}
              className="flex items-center justify-center text-white text-[11px] font-bold transition-all hover:opacity-80"
              style={{ flex: count, background: STAGE_COLORS[s] }}
      ...
      <div className="flex gap-3 overflow-x-auto pb-4" style={{ minHeight: "60vh" }}>
        {STAGES.map((stage) => {
          const brands = (grouped[stage] || []).filter(
            (b) =>
              !q ||
              b.name.toLowerCase().includes(q) ||
              (b.salesRep && b.salesRep.toLowerCase().includes(q)),
          );
          return (
            <div key={stage} className="flex-shrink-0 w-64">
              ...
              <div className="bg-slate-100 rounded-b-lg p-2 space-y-2 min-h-[200px] max-h-[70vh] overflow-y-auto">
                {brands.map((b) => (
                  <div
                    key={b.id}
                    className="bg-white rounded-lg p-3 shadow-sm border border-slate-200 hover:shadow-md hover:border-blue-300 transition-all cursor-pointer group relative"
                  >
                    <div onClick={() => router.push(`/brands/${b.id}`)}>
                      <div className="font-bold text-sm text-slate-900 truncate pr-6">{b.name}</div>
                      {b.salesRep && (
                        <div className="text-[11px] text-slate-500 mt-1 truncate">{b.salesRep}</div>
                      )}
                      <div className="flex gap-3 mt-2 text-[11px] text-slate-500">
                        {b.fabricCount > 0 && (
                          <span title={t.brands.fabrics}>🧵 {b.fabricCount}</span>
                        )}
                        {b.submissionCount > 0 && (
                          <span title={t.brands.submissions}>📋 {b.submissionCount}</span>
                        )}
                        {b.factoryCount > 0 && (
                          <span title={t.brands.factories}>🏭 {b.factoryCount}</span>
                        )}
                      </div>
                    </div>
                ...
                {brands.length === 0 && (
                  <div className="text-xs text-slate-400 text-center py-8">{t.brands.noBrands}</div>
                )}
              </div>
            </div>
          );
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-sm p-6">
            ...
            {deleteTarget.fabricCount > 0 ||
            deleteTarget.submissionCount > 0 ||
            deleteTarget.factoryCount > 0 ? (
              <p className="text-xs text-amber-600 mb-4">
                {t.brands.hasLinkedRecords}{" "}
                {[
                  deleteTarget.fabricCount > 0 &&
                    `${deleteTarget.fabricCount} ${t.brands.fabrics.toLowerCase()}`,
                  deleteTarget.submissionCount > 0 &&
                    `${deleteTarget.submissionCount} ${t.brands.submissions.toLowerCase()}`,
                  deleteTarget.factoryCount > 0 &&
                    `${deleteTarget.factoryCount} ${t.brands.factories.toLowerCase()}`,
                ]
                  .filter(Boolean)
                  .join(", ")}
                . {t.brands.deleteMayFail}
              </p>
            ) : (
