    ...
    <div className="max-w-7xl mx-auto px-4 py-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <h1 className="text-2xl font-black text-slate-800">Sample → Production Conversion</h1>
          <p className="text-sm text-slate-500 mt-1">
            Track which factories received samples and converted to production orders
          </p>
        </div>
      </div>
      ...
      {summary && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
          <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 text-center">
            ...
            <p className="text-[10px] font-semibold text-blue-600 uppercase">Factories Sampled</p>
          </div>
          <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 text-center">
            <p className="text-2xl font-black text-emerald-700">{summary.totalConverted}</p>
            <p className="text-[10px] font-semibold text-emerald-600 uppercase">Converted</p>
          </div>
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-center">
            <p className="text-2xl font-black text-amber-700">{summary.conversionRate}%</p>
            <p className="text-[10px] font-semibold text-amber-600 uppercase">Conversion Rate</p>
          </div>
          <div className="bg-cyan-50 border border-cyan-200 rounded-xl p-4 text-center">
            ...
            <p className="text-[10px] font-semibold text-cyan-600 uppercase">Avg Days to Convert</p>
          </div>
        </div>
      )}
      ...
      {summary && (
        <div className="grid grid-cols-2 gap-3 mb-6">
          <div className="bg-white border rounded-xl p-4 text-center">
            ...
            <p className="text-[10px] font-semibold text-slate-500 uppercase">Production Volume</p>
          </div>
          <div className="bg-white border rounded-xl p-4 text-center">
            <p className="text-xl font-black text-slate-700">${summary.totalProductionRevenue.toLocaleString()}</p>
            <p className="text-[10px] font-semibold text-slate-500 uppercase">Production Revenue</p>
          </div>
        </div>
      )}
      ...
      <div className="flex gap-2 mb-4">
        {(["all", "converted", "pending"] as const).map((t) => (
          ...
          >
            {t === "all" ? `All (${(converted.length + pending.length)})` : t === "converted" ? `Converted (${converted.length})` : `Pending (${pending.length})`}
          </button>
        ))}
      </div>
      ...
      <div className="space-y-2">
        {factories.length === 0 ? (
          <div className="text-center py-12 bg-white border rounded-xl">
            <p className="text-slate-400 text-sm">No sample orders found yet</p>
          </div>
        ) : (
          factories.map((f) => (
            <div key={f.factoryId} className="bg-white border rounded-xl overflow-hidden">
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {f.converted ? (
                      <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-emerald-100 text-emerald-700">CONVERTED</span>
                    ) : (
                  ...
                  <p className="text-xs text-slate-500">
                    ...
                    {f.daysToConvert !== null && ` · ${f.daysToConvert} days to convert`}
                    {f.accountManager && f.accountManager !== "—" && ` · AM: ${f.accountManager}`}
                  </p>
                </div>
                {f.converted && (
              ...
              {expanded.has(f.factoryId) && (
                <div className="border-t bg-slate-50 p-4 space-y-3">
                  {f.samples.length > 0 && (
                    <div>
                      <p className="text-xs font-bold text-slate-500 uppercase mb-1">Sample Orders</p>
                      {f.samples.map((s, i) => (
                  ...
                  {f.trials.length > 0 && (
                    <div>
                      <p className="text-xs font-bold text-slate-500 uppercase mb-1">Trial Requests</p>
                      {f.trials.map((t, i) => (
                        <div key={i} className="flex items-center gap-3 text-sm py-1">
                          <span className="text-slate-400 text-xs">{new Date(t.date).toLocaleDateString()}</span>
                          <span className="font-semibold text-slate-700">{t.brandName}</span>
                          <span className="text-xs text-slate-500">{t.volume}L</span>
                          <span className={`px-1.5 py-0.5 rounded text-[9px] font-bold ${t.status === "COMPLETE" ? "bg-green-100 text-green-700" : "bg-blue-100 text-blue-700"}`}>
                  ...
                  {f.productionOrders.length > 0 && (
                    <div>
                      <p className="text-xs font-bold text-emerald-600 uppercase mb-1">Production Orders</p>
