...
export default function NewFabricPage() {
  ...
  return (
    <div className="max-w-[800px] mx-auto">
      <button onClick={() => router.push("/fabrics")} className="text-sm text-blue-600 hover:underline mb-2 block">&larr; {t.fabrics.backToFabrics}</button>
      <h1 className="text-2xl font-black text-slate-900 mb-6">{t.fabrics.addNewFabric}</h1>
      {error && <div className="mb-4 p-3 bg-red-50 border border-red-200 rounded-lg text-red-700 text-sm">{error}</div>}
      <form onSubmit={handleSubmit} className="bg-white rounded-xl p-6 shadow-sm border space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-xs font-semibold text-slate-500 mb-1">{t.fabrics.fuzeNumber}</label>
            <input type="number" value={form.fuzeNumber} onChange={e => set("fuzeNumber", e.target.value)}
              className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
          <div>
            <label className="block text-xs font-semibold text-slate-500 mb-1">{t.fabrics.customerCode}</label>
            <input type="text" value={form.customerCode} onChange={e => set("customerCode", e.target.value)}
              className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
          <div>
            <label className="block text-xs font-semibold text-slate-500 mb-1">{t.fabrics.factoryCode}</label>
            <input type="text" value={form.factoryCode} onChange={e => set("factoryCode", e.target.value)}
              className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
          <div>
            <label className="block text-xs font-semibold text-slate-500 mb-1">{t.fabrics.construction}</label>
            <input type="text" value={form.construction} onChange={e => set("construction", e.target.value)} placeholder="e.g. Knit, Woven, Jersey"
              className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
          <div>
            <label className="block text-xs font-semibold text-slate-500 mb-1">{t.fabrics.color}</label>
            <input type="text" value={form.color} onChange={e => set("color", e.target.value)}
              className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
          <div>
            <label className="block text-xs font-semibold text-slate-500 mb-1">{t.fabrics.weightGsm}</label>
            <input type="number" step="0.1" value={form.weightGsm} onChange={e => set("weightGsm", e.target.value)}
              className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
          <div>
            <label className="block text-xs font-semibold text-slate-500 mb-1">{t.fabrics.widthInches}</label>
            <input type="number" step="0.1" value={form.widthInches} onChange={e => set("widthInches", e.target.value)}
              className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
          <div>
            <label className="block text-xs font-semibold text-slate-500 mb-1">{t.fabrics.yarnType}</label>
            <input type="text" value={form.yarnType} onChange={e => set("yarnType", e.target.value)}
              className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
          <div>
            <label className="block text-xs font-semibold text-slate-500 mb-1">{t.fabrics.brand}</label>
            <select value={form.brandId} onChange={e => set("brandId", e.target.value)}
              className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
              <option value="">{t.fabrics.selectBrand}</option>
              {brands.map(b => <option key={b.id} value={b.id}>{b.name}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-xs font-semibold text-slate-500 mb-1">{t.fabrics.factory}</label>
            <select value={form.factoryId} onChange={e => set("factoryId", e.target.value)}
              className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
              <option value="">{t.fabrics.selectFactory}</option>
        ...
        <div>
          <label className="block text-xs font-semibold text-slate-500 mb-1">{t.fabrics.finishNote}</label>
        ...
        {/* Content composition */}
        <div>
          <label className="block text-xs font-semibold text-slate-500 mb-2">{t.fabrics.fabricContent}</label>
          {contents.map((c, i) => (
            <div key={i} className="flex gap-2 mb-2">
              <input type="text" placeholder={t.fabrics.materialPlaceholder} value={c.material}
          ...
          <button type="button" onClick={() => setContents([...contents, { material: "", percent: "" }])} className="text-xs text-blue-600 hover:underline">+ {t.fabrics.addMaterial}</button>
        </div>

        <div className="flex justify-end gap-3 pt-4 border-t">
          <button type="button" onClick={() => router.push("/fabrics")} className="px-4 py-2 bg-slate-200 text-slate-700 rounded-lg text-sm font-semibold">{t.common.cancel}</button>
          <button type="submit" disabled={saving} className="px-6 py-2 bg-blue-600 text-white rounded-lg text-sm font-semibold hover:bg-blue-700 disabled:opacity-50">
            {saving ? t.common.creating : t.fabrics.createFabric}
          </button>
        </div>
      </form>
    </div>
