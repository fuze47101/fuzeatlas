// @ts-nocheck
/**
 * ACM Task CRUD (list + create).
 *
 *   GET  /api/acm/tasks?status=OPEN&brandId=...&mine=true&window=overdue|today|week
 *   POST /api/acm/tasks { title, notes?, dueAt, ownerId?, brandId?, priority? }
 *
 * Powers the My Tasks widget, the /acm/tasks page, and the +Task button on
 * brand detail pages. Reminder emails are sent by /api/cron/task-reminders.
 *
 * Renamed from /api/crm/tasks on 2026-04-20 as part of the CRM → ACM
 * (Atlas Customer Management) rebrand. A 308-redirect shim at
 * /api/crm/tasks keeps stale clients working.
 */
