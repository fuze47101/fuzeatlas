...
/**
 * /api/lab-portal/team — NEED-7a.
 *
 * GET:    list teammates + PENDING invitations on the caller's lab.
 * POST:   send a new invitation (LAB_USER + admin only).
 * DELETE: revoke a PENDING invitation by id.
 */

const ADMIN_ROLES = ROSTER_ADMIN_ROLES.LAB;
...
export async function GET() {
  ...
  const { teammates, pending } = await listOrgTeam("LAB", lab.id);
...
export async function DELETE(req: Request) {
  ...
  if (!invite || invite.entityType !== "LAB" || invite.entityId !== user.labId) {
